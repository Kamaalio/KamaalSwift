...
public struct Shell {
    ...
    /// Executes an AppleScript command using `osascript` and returns the standard output or an error.
    /// - Parameters:
    ///   - script: The AppleScript command or script to run.
    ///   - executionLocation: Optional working directory to `cd` into before executing.
    ///   - environmentVariables: Dictionary of environment variables to set for the command execution.
    /// - Returns: `.success(stdout)` when the script succeeds, or `.failure` when an error occurs.
    ///
    /// # Example
    /// ```swift
    /// let result = Shell.appleScript("display dialog \"Hello World\"")
    /// if case let .success(output) = result { print(output) }
    ///
    ...
    ///     end tell
    /// """
    /// let diskResult = Shell.appleScript(script)
    /// ```
    ...
    public static func shell(
        ...
        let result = self.executeProcess(
            launchPath: launchPath,
            arguments: ["-c", commandToUse],
            at: nil, // We handle directory changes manually above
            withEnvironmentVariables: environmentVariables,
        )
    ...
    /// Executes a process with custom arguments and returns the standard output or an error.
    /// - Parameters:
    ///   - launchPath: Absolute path to an executable (e.g., `/usr/bin/osascript`).
    ...
    private static func executeProcess(
        ...
        if let executionLocation {
            task.currentDirectoryURL = URL(fileURLWithPath: executionLocation)
        }
